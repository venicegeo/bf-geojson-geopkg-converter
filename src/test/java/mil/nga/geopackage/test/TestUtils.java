...
public class TestUtils {
	...
	public static Point createPoint(boolean hasZ, boolean hasM) {
		...
		double x = Math.random() * 180d * (Math.random() < .5 ? 1 : -1);
		double y = Math.random() * 90d * (Math.random() < .5 ? 1 : -1);

		Point point = new Point(x, y, hasZ ? Math.random() * 1000d : null, hasM ? Math.random() * 1000d : null);
